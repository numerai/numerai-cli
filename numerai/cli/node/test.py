...
def test(ctx, local, command, verbose):
    ...
    try:
        if 'cron' in node_config:
            ...
            if provider == 'aws':
                ...
                exit(1)
            click.secho(
                f"Trigger ID assigned for this test: {trigger_id}", fg='green')
    ...
    if node_config['provider'] == 'azure':
        click.secho("[Azure node] Test complete, your model should submits automatically! "
                    "You may check your submission here: https://numer.ai/models", fg='green')
        return
...
def monitor_azure(node, config, verbose):
    ...
    if len([keys for keys in storage_keys.keys]) == 0:
        click.secho(
            f"Webhook's storage account key not found, check storage account name: {storage_account_name}", fg='red')
        exit(1)
    ...
    table_name = [table.name for table in table_service_client.list_tables(
    ) if 'History' in table.name][0]
    ...
    while time_lapse <= timedelta(minutes=15) and monitoring_done == False:
        if len(shown_log_row_key) == 0 and verbose:
            ...
            click.secho(
                f"Waiting for submission run to finish...\r", fg='yellow',)
...
def azure_refresh_and_print_log(table_client, monitor_start_time, shown_log_row_key=list()):
    ...
    log_df = log_df[log_df['RowKey'].isin(
        shown_log_row_key) == False].reset_index()
    ...
    if len(log_df) > 0:
        for i in range(len(log_df)):
            ...
            if log_df.loc[i, 'EventType'] == 'ExecutionStarted':
                ...
                az_func_name1 = execution_log.loc[execution_log['EventType']
                                                  == 'ExecutionStarted', 'Name'].values[0]
                execute_st = execution_log.loc[execution_log['EventType']
                                               == 'ExecutionStarted', '_Timestamp'].values[-1]
                execute_et = execution_log.loc[execution_log['EventType']
                                               == 'ExecutionCompleted', '_Timestamp'].values[-1]
                ...
                click.secho(
                    f"'{az_func_name1}' result: {log_df.loc[i,'Result']}")
