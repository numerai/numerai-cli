...
def doctor():
    ...
    if sys.platform == "linux" or sys.platform == "linux2":
        env_setup_cmd = (
            "sudo apt update && sudo apt install -y libcurl4 curl && "
        ...
        env_setup_cmd = (
            'powershell -command "$Script = Invoke-WebRequest '
        ...
        env_setup_status = 1
        env_setup_err = (
            f"Unrecognized Operating System {sys.platform}, "
            f"cannot run environment setup script, skipping..."
        )

    if env_setup_cmd is not None:
        res = subprocess.run(
            env_setup_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
    ...
    if env_setup_status != 0:
        ...
        click.secho(
            ...
            "https://github.com/numerai/numerai-cli/wiki/Troubleshooting",
            fg="red",
        )
    else:
    ...
    if curr_ver < versions[0]:
        click.secho(
            "numerai-cli needs an upgrade" "(run `pip3 install -U numerai-cli` to fix)",
            fg="red",
        )
    else:
